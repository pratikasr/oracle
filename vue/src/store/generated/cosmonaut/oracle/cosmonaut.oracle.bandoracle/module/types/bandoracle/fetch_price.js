...
const baseFetchPriceCallData = { symbols: '', multiplier: 0 };
export const FetchPriceCallData = {
    ...
    decode(input, length) {
        ...
        const message = { ...baseFetchPriceCallData };
        message.symbols = [];
...
const baseFetchPriceResult = { rates: 0 };
export const FetchPriceResult = {
    ...
    decode(input, length) {
        const reader = input instanceof Uint8Array ? new Reader(input) : input;
        let end = length === undefined ? reader.len : reader.pos + length;
        const message = { ...baseFetchPriceResult };
        message.rates = [];
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
